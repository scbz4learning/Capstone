...
def profile_llamacpp(config, args):
    ...
    for cfg in configs:
        ...
        try:
            ...
            cpu_mem_monitor = CpuMemoryMonitor(pid=server_pid, interval_s=0.5)
            ...
            # === Memory: continuous monitors updated during latency + power ===
            ...
            # Stop monitors and capture peaks
            gpu_peak_vram_mb = gpu_mem_monitor.stop()
            peak_cpu_rss_mb = cpu_mem_monitor.stop()

            # Reconstruct memory details from continuous monitors
            peak_mem_allocated_gb = round(peak_cpu_rss_mb / 1024, 2)
