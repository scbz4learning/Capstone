...
DEFAULT_CONFIG = {
    ...
    "target_metrics": {
        ...
        "end_to_end": ["latency_ms", "batch_throughput_img_per_sec", "peak_mem"],
        "power": ["avg_power", "energy_per_inference", "img_per_sec_watt"]
    }
}
...
def profile_vggt(config, args):
    ...
    for cfg in configs:
        ...
        try:
            ...
            latency_stats = calculate_metrics(latency_list)
            energy_per_step_j = total_energy_j / max(1, NUM_TEST_POWER) if not is_wsl_worker else 0
            batch_throughput_img_per_sec = num_images / (latency_stats["mean"] / 1000.0) if latency_stats["mean"] > 0 else 0
            img_per_sec_watt = batch_throughput_img_per_sec / avg_power_adjusted if avg_power_adjusted > 0 else 0
            ...
            power_struct = {
                ...
                "img_per_sec_watt": img_per_sec_watt,
            ...
            res = {
                ...
                "latency": {
                    ...
                    "latency_ms_p50": latency_stats["median"],
                    "latency_ms_p95": latency_stats["p95"],
                    "batch_throughput_img_per_sec": batch_throughput_img_per_sec
                },
            ...
            print(f"Results: Latency {latency_stats['mean']:.1f}ms (Img/s: {batch_throughput_img_per_sec:.2f}) | Power {avg_power:.1f}W | Energy/Step {energy_per_step_j:.2f}J")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Profile VGGT Model with dynamic metrics")
